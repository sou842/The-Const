      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-card/20 backdrop-blur-3xl animate-in fade-in zoom-in-95 duration-500">
        <div className="w-full max-w-sm space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button
              variant="outline"
              className="h-11 rounded-xl border-muted-foreground/10 hover:bg-muted/50 transition-colors"
              onClick={() => {
                const callbackUrl = searchParams.get("callbackUrl") || "/";
            ...
            <Button
              ...
              onClick={() => {
                const callbackUrl = searchParams.get("callbackUrl") || "/";
                window.location.href = `/api/auth/oauth/github?callbackUrl=${encodeURIComponent(callbackUrl)}`;
              }}
              type="button"
            >
